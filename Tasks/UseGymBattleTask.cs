...
namespace PoGo.NecroBot.Logic.Tasks
{
    public class UseGymBattleTask
    {
        ...
        private static async Task StartGymAttackLogic()
        {
            ...
            Logger.Write("Defending Team: " + string.Join(", ", defenders.Select(d => string.Format("{0} (CP:{1} | Lvl: {2})", d.PokemonId.ToString(), d.CP(), d.Level()))),LogLevel.Gym);
            ...
            {
                Logger.Write("Attacking Team: " + string.Join(", ", _session.GymState.MyTeam.Select(a => string.Format("{0} (CP:{1} | Lvl: {2} | HP:{3})", a.Attacker.PokemonId, a.Attacker.Level(), a.Attacker.Cp, a.HpState))), LogLevel.Gym, ConsoleColor.White);
        ...
        private static PokemonData GetBestInBattle(PokemonData defender)
        {
            ...
            if (newAttacker != null)
                Logger.Write(string.Format("New atacker to switch will be {0} {1} CP {2}", newAttacker.PokemonId, newAttacker.Cp, newAttacker.Id), LogLevel.Gym, ConsoleColor.Green);
        ...
        private static async Task<List<BattleAction>> AttackGym(GymStartSessionResponse startResponse, int counter)
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    Logger.Write(string.Format("Going to make attack : {0}", string.Join(", ", attackActionz.Select(s => string.Format("{0} -> {1}", s.Type, s.DurationMs)))), LogLevel.Gym, ConsoleColor.White);
                    ...
                    try
                    {
                        ...
                        {
                            Logger.Write("Etra wait before SWAP call", LogLevel.Gym, ConsoleColor.White);
                        ...
                        Logger.Write(string.Format("Waiting for attack to be prepared: {0} (last call was {1}, after correction {2})", attackTime, timeAfter, attackTimeCorrected > 0 ? attackTimeCorrected : 0), LogLevel.Gym, ConsoleColor.White);
                        if (attackTimeCorrected > 0)
                            await Task.Delay(attackTimeCorrected).ConfigureAwait(false);

                        if (attackActionz.Any(a => a.Type == BattleActionType.ActionSwapPokemon))
                        {
                            Logger.Write("Etra wait after SWAP call", LogLevel.Gym, ConsoleColor.White);
                            await Task.Delay(2000).ConfigureAwait(false);
                        }
                    }
                    catch (APIBadRequestException)
                    ...
                    if (attackResult.Result == GymBattleAttackResponse.Types.Result.Success)
                    {
                        ...
                        switch (attackResult.BattleUpdate.BattleLog.State)
                        {
                            case BattleState.Active:
                                ...
                                Logger.Write($"(DEFENDER): {defender.PokemonId.ToString(),-12} | HP: {attackResult.BattleUpdate.ActiveDefender.CurrentHealth,3:##0} | Sta: {attackResult.BattleUpdate.ActiveDefender.CurrentEnergy,3:##0} | Lvl: {attackResult.BattleUpdate.ActiveDefender.PokemonData.Level(),4:#0.0} | CP: {attackResult.BattleUpdate.ActiveDefender.PokemonData.CP(),4:###0}", LogLevel.Gym,
                                    (ev == TeamColor.Red) ? ConsoleColor.Red : (ev == TeamColor.Yellow ? ConsoleColor.Yellow : ConsoleColor.Blue));

                                Logger.Write($"(ATTACKER): {attacker.PokemonId.ToString(),-12} | HP: {attackResult.BattleUpdate.ActiveAttacker.CurrentHealth,3:##0} | Sta: {attackResult.BattleUpdate.ActiveAttacker.CurrentEnergy,3:##0} | Lvl: {attackResult.BattleUpdate.ActiveAttacker.PokemonData.Level(),4:#0.0} | CP: {attackResult.BattleUpdate.ActiveAttacker.PokemonData.CP(),4:###0}", LogLevel.Gym,
                                    (player.Team == TeamColor.Red) ? ConsoleColor.Red : (player.Team == TeamColor.Yellow ? ConsoleColor.Yellow : ConsoleColor.Blue));
            ...
            if (attacker != null && defender != null)
            {
                ...
                else
                {
                    ...
                    if (canDoSpecialAttack)
                    {
                        ...
                        Logger.Write(string.Format("Trying to make an special attack {0}, on: {1}, duration: {2}", specialMove.MovementId, serverMs, specialMove.DurationMs), LogLevel.Gym, ConsoleColor.White);
                    }
                    else if (canDoAttack)
                    {
                        ...
                        Logger.Write(string.Format("Trying to make an normal attack {0}, on: {1}, duration: {2}", normalMove.MovementId, serverMs, normalMove.DurationMs), LogLevel.Gym, ConsoleColor.White);
                    }
                    else
                    {
